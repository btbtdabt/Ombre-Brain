...
def _service(tmp_path, **config):
    ...
    setattr(service, "dehydrator", ExplodingLLM())
...
def _required_person(service, scope, name):
    person = service.store.find_person_by_name(scope, name)
    assert person is not None
    return person


def _required_claim(service, scope, claim_id):
    claim = service.store.get_claim(scope, claim_id)
...
class TestRenameByHuman:
    ...
    async def test_改名之后下次浮现提醒一次(self, tmp_path):
        ...
        person = _required_person(service, scope, "Zoey")
    ...
    async def test_认识正文一个字都不动(self, tmp_path):
        ...
        person = _required_person(service, scope, "Zoey")
        service.rename_person(person.id, ["Zoey Chen"])
        after = _required_claim(service, scope, claim.id)
    ...
    async def test_称呼撞车被拒(self, tmp_path):
        ...
        scope = _scope(service)
        阿哲 = _required_person(service, scope, "阿哲")
...
class TestPersons:
    ...
    async def test_新称呼会并进已有的人(self, tmp_path):
        ...
        scope = _scope(service)
        person = _required_person(service, scope, "阿 Z")
...
class TestRecall:
    ...
    async def test_命中算被提起一次(self, tmp_path):
        ...
        await self._formalized(service)
        scope = _scope(service)
        before = _required_person(service, scope, "Zoey").activation_count
        await service.recall(query="Zoey")
        after = _required_person(service, scope, "Zoey").activation_count
...
class TestDelete:
    ...
    async def test_撤回不需要三次确认(self, tmp_path):
        ...
        after = _required_claim(service, _scope(service), claim.id)
