...
def test_restore_invalidates_p0_bucket_cache(tmp_path):
    source_config = _config(tmp_path / "source-cache")
    source_manager = BucketManager(source_config)
    bucket_id = asyncio.run(
        source_manager.create(content="restored content", name="Shared", bucket_id="shared-cache")
    )
    source_backup = VaultBackupManager(
        source_config,
        source_manager,
    ...
    target_config = _config(tmp_path / "target-cache")
    target_manager = BucketManager(target_config)
    asyncio.run(
        target_manager.create(content="cached local content", name="Shared", bucket_id=bucket_id)
    )
    primed = asyncio.run(target_manager.list_all())
    assert primed[0]["content"] == "cached local content"
    ...
    try:
        ...
        refreshed = asyncio.run(target_manager.list_all())
    finally:
        Path(archive_path).unlink(missing_ok=True)

    assert refreshed[0]["content"] == "restored content"
...
def test_backup_round_trip_restores_persisted_media(tmp_path):
    source_config = _config(tmp_path / "source-media")
    source_manager = BucketManager(source_config)
    payload = b"persistent-photo"
    bucket_id = asyncio.run(
        source_manager.create(
            content="memory with media",
            bucket_id="media-memory",
            media={
    ...
    archive_path, manifest = source_backup.create_archive()

    target_config = _config(tmp_path / "target-media")
    target_manager = BucketManager(target_config)
    target_backup = VaultBackupManager(
        target_config,
        target_manager,
    ...
    try:
        restored = asyncio.run(target_backup.restore_archive(archive_path))
    finally:
        Path(archive_path).unlink(missing_ok=True)

    assert any(
        str(file_record.get("path") or "").startswith("media/")
        for file_record in manifest["files"]
    )
    assert restored["created"] == 1
    bucket = asyncio.run(target_manager.get(bucket_id))
    assert bucket is not None
    media_path = (
        Path(target_config["buckets_dir"])
        / bucket["metadata"]["media"][0]["path"]
    )
    assert media_path.read_bytes() == payload


def test_backup_rejects_silent_media_omission_with_legacy_archive_builder(
    ...
    asyncio.run(
        manager.create(
            content="memory with media",
            media={
                "data_base64": base64.b64encode(b"persistent-photo").decode("ascii"),
                "filename": "photo.png",
                "type": "image/png",
            },
        )
    ...
    def builder_without_media(buckets_dir, embedding_db_path, export_meta):
        raise AssertionError("media-blind archive builder must not be called")

    monkeypatch.setattr(
        "backup_manager.build_export_archive_file", builder_without_media
    )

    with pytest.raises(BackupArchiveError, match="媒体"):
