...
def test_tool_description_frames_quotes_as_my_own_question():
    ...
    但至少能挡住最直白的那种。
    """
    from tools.current.memory import breath_search, hold, trace

    for tool in (breath_search, hold, trace):
...
def test_write_side_description_says_the_default_is_none():
    ...
    doc = _doc_of(getattr(server, "hold"))
...
def test_read_side_description_denies_a_full_text_entry():
    ...
    doc = _doc_of(getattr(server, "breath_search"))
    assert "不是原文" in doc
    assert "全文" in doc
...
def test_trace_description_states_the_no_backfill_boundary():
    ...
    doc = _doc_of(getattr(server, "trace"))
