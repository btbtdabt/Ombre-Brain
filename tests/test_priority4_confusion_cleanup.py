...
async def test_dashboard_config_applies_zero_dehydration_temperature(monkeypatch):
    ...
    monkeypatch.setattr(
        config_api, "read_config_yaml", lambda: copy.deepcopy(persisted)
    )
    monkeypatch.setattr(config_api, "atomic_update_config_yaml", persist)
    mcp = FakeMCP()
    config_api.register(mcp)
    ...
    response = await mcp.routes[("POST", "/api/config")](
        JsonRequest({"dehydration": {"temperature": 0.0}, "persist": True})
    )

    assert response.status_code == 200
    assert runtime["dehydration"]["temperature"] == 0.0
    assert persisted["dehydration"]["temperature"] == 0.0
    assert dehydrator.temperature == 0.0


@pytest.mark.asyncio
async def test_dashboard_config_yaml_failure_rolls_back_runtime_objects(monkeypatch):
    runtime = {"dehydration": {"temperature": 0.3}}
    dehydrator = SimpleNamespace(
        model="model",
        base_url="https://example.invalid/v1",
    ...
    monkeypatch.setattr(config_api.sh, "dehydrator", dehydrator, raising=False)
    monkeypatch.setattr(
        config_api, "read_config_yaml", lambda: copy.deepcopy(runtime)
    )
    monkeypatch.setattr(
        config_api,
        "atomic_update_config_yaml",
    ...
    assert response.status_code == 500
    assert runtime["dehydration"]["temperature"] == 0.3
    assert dehydrator.temperature == 0.3
...
@pytest.mark.asyncio
async def test_timezone_hot_apply_updates_date_parser_without_persistence(
    monkeypatch, tmp_path
):
    config_path = tmp_path / "config.yaml"
    runtime = {"transport": "streamable-http"}
    ...
    monkeypatch.setattr(config_api.sh, "in_docker", lambda: False)
    monkeypatch.setattr(utils, "config_file_path", lambda: str(config_path))
    monkeypatch.setattr(utils, "_runtime_timezone_name", None)
    mcp = FakeMCP()
    config_api.register(mcp)

    response = await mcp.routes[("POST", "/api/config")](
        JsonRequest({"timezone": "America/New_York"})
    )

    assert response.status_code == 200
    assert not config_path.exists()
    assert utils.get_timezone_name() == "America/New_York"
    assert getattr(utils.get_tzinfo(), "key", "") == "America/New_York"
