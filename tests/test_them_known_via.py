...
def _required_claim(service, claim_id):
    claim = service.store.get_claim(_scope(service), claim_id)
...
def _person(service):
    return service.store.list_persons(_scope(service))[0]
...
async def test_human_registered_person_is_heard_from_user_and_visible(tmp_path):
    ...
    person = service.store.list_persons(_scope(service))[0]
...
async def test_recall_reports_the_field_not_the_derivation(tmp_path):
    ...
    for _ in range(REQUIRED_CONFIRMATIONS):
        ...
        claim = _age_receipts(service, _required_claim(service, claim.id))
    await _write(service, known_via=KNOWN_VIA_HEARD_FROM_USER)
