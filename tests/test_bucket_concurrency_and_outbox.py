...
def test_ensure_embedding_index_uses_title_and_content(tmp_path, monkeypatch):
    manager = BucketManager(_config(tmp_path))
    captured = []

    async def capture_index(bucket_id, text):
        captured.append((bucket_id, text))
        return True

    monkeypatch.setattr(manager, "_index_after_write", capture_index)

    async def scenario():
        bucket_id = await manager.create(
            content="The remembered body",
            name="A distinctive title",
        )
        captured.clear()
        assert await manager.ensure_embedding_index(bucket_id) is True
        bucket = await manager.get(bucket_id)
        assert bucket is not None
        assert captured == [(bucket_id, bucket_text_for_embedding(bucket))]
        assert "A distinctive title" in captured[0][1]
        assert "The remembered body" in captured[0][1]
