...
def _letter_id(receipt: str) -> str:
    return receipt.split("→", 1)[1].split(" ", 1)[0]
...
@pytest.mark.asyncio
@pytest.mark.parametrize("extra", [{"date": "2026-08-01"}, {"anchor": 0}])
async def test_current_trace组合修改不会把title写进name(环境, extra):
    from tools.current.memory import trace as current_trace

    管理器, 编号 = 环境
    修改前 = await 管理器.get(编号)
    原桶名 = 修改前["metadata"]["name"]

    await current_trace(bucket_id=编号, title="组合修改的标题", **extra)

    修改后 = await 管理器.get(编号)
    assert 修改后["metadata"]["title"] == "组合修改的标题"
    assert 修改后["metadata"]["name"] == 原桶名
...
async def test_信件的标题改得动(环境):
    ...
    管理器, _ = 环境
    回执 = await letter_write(author="ai", content="一封普通的信。", title="原来的信题")
    信 = await 管理器.get(_letter_id(回执))
    assert 信 is not None
