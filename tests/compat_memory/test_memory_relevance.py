import memory_relevance as relevance_module
...
def test_stopword_loader_treats_an_undecodable_resource_as_missing(tmp_path):
    resource = tmp_path / "stopwords.txt"
    resource.write_bytes(b"\xff")

    assert relevance_module._load_stopword_file(resource) == set()


def test_facet_cache_keys_use_option_contents_not_recycled_object_ids(monkeypatch):
    first = MemoryRelevanceOptions(aliases={"first": ("unique-token",)})
    second = MemoryRelevanceOptions(aliases={"second": ("unique-token",)})
    relevance_module._FACETS_FOR_TEXT_CACHE.clear()
    monkeypatch.setattr(relevance_module, "id", lambda _value: 1, raising=False)

    try:
        assert facets_for_text("unique-token", first) == {"first": 0.65}
        assert facets_for_text("unique-token", second) == {"second": 0.65}
    finally:
        relevance_module._FACETS_FOR_TEXT_CACHE.clear()
