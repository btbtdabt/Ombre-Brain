...
def test_tool_input_limits_reject_oversize_before_side_effects(monkeypatch):
    ...
    grow_error = check_grow_input_size("x" * 11)
    query_error = check_query_size("查询" * 5)
    items_error = check_grow_items_payload(["a", "b", "c"])

    assert grow_error is not None
    assert query_error is not None
    assert items_error is not None
    assert "grow 输入过大" in grow_error
    assert "查询过大" in query_error
    assert "items 过多" in items_error
...
async def test_mcp_body_limit_rejects_declared_and_chunked_payloads():
    ...
    await middleware(declared_scope, declared_receive, send)
