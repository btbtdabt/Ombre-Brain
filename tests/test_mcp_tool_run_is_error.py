...
@pytest.mark.asyncio
async def test_letter正文超限沿用P0错误契约且不建桶(工具):
    """Union wrapper 不能在 P0 letter_write 前把校验失败 return 成成功字符串。"""
    取, 管理器 = 工具
    前 = await _桶数(管理器)
    with pytest.raises(ToolError, match="内容过大"):
        await 取("letter_write").run(
            {"author": "user", "content": "长" * 5_000_000}
        )
    assert await _桶数(管理器) == 前


@pytest.mark.asyncio
async def test_letter读取查询超限是错误(工具):
    取, _ = 工具
    with pytest.raises(ToolError, match="查询过大"):
        await 取("letter_read").run({"query": "长" * 100_000})


@pytest.mark.asyncio
async def test_letter读取作者超限是错误(工具):
    取, _ = 工具
    with pytest.raises(ToolError, match="元数据过大"):
        await 取("letter_read").run({"author": "长" * 100_000})
...
@pytest.mark.asyncio
async def test_trace当前扩展分支正文超限仍是错误且不改桶(工具):
    """anchor/date 扩展分支也必须保持 P0 的 ToolInputError 语义。"""
    取, 管理器 = 工具
    编号 = await 管理器.create(content="原正文。", title="扩展分支靶子")
    with pytest.raises(ToolError, match="内容过大"):
        await 取("trace").run(
            {"bucket_id": 编号, "anchor": 0, "content": "长" * 5_000_000}
        )
    桶 = await 管理器.get(编号)
    assert 桶["content"] == "原正文。"
    assert not 桶["metadata"].get("anchor")
