...
async def test_降级提示不受影响仍算成功(hold_tool, monkeypatch):
    ...
    async def 成功但附带提示(**kwargs):
        return "abc123", False, "👣 Footprint：暂时无法读取"

    async def 无副作用(*_args, **_kwargs):
        return None

    # The integrated fork keeps the current-production adapter as the public
    # handler; patch its shared merge boundary instead of P0's direct store.
    monkeypatch.setattr(current_memory, "merge_or_create", 成功但附带提示)
    monkeypatch.setattr(current_memory, "check_duplicate_for", 无副作用)
    monkeypatch.setattr(current_memory, "check_plan_resolution", 无副作用)
    out = await hold_tool.run({"content": "一条正常的记忆"})
    assert "新建" in str(out)
    assert "Footprint" in str(out)
