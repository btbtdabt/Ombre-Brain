...
def install_dashboard_runtime(bucket_mgr, monkeypatch):
    ...
    monkeypatch.setattr(
        letters.sh,
        "letter_service",
        LetterService(bucket_mgr.config, bucket_mgr, DisabledEmbedding()),
    )
...
def test_naive_unlock_date_uses_configured_timezone(monkeypatch, tmp_path):
    ...
    # 纯日期
    normalized = normalize_unlock_date("timed", "2030-01-01")
    assert normalized is not None
    assert normalized.startswith("2030-01-01T00:00:00+08:00")
    # 无时区的完整时刻
    normalized = normalize_unlock_date("timed", "2030-01-01T09:30:00")
    assert normalized is not None
    assert normalized.startswith("2030-01-01T09:30:00+08:00")
    # 显式时区优先于配置
    normalized = normalize_unlock_date("timed", "2030-01-01T00:00:00+00:00")
    assert normalized is not None
    assert normalized.startswith("2030-01-01T00:00:00+00:00")
...
async def test_dashboard_creates_human_lock_hides_it_from_ai_and_rejects_ai_proxy(
    ...
    monkeypatch.setenv("AI_NAME", "张三")
    install_dashboard_runtime(bucket_mgr, monkeypatch)
    mcp = FakeMCP()
