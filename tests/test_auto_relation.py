...
import tools._relation_link as relation_link
...
from tools.relation_bindings import detach, restore
...
def test_detached_auto_history_does_not_consume_active_link_budget():
    detached = [
        {
            **_auto(f"old-{index}", 0.9),
            "status": "detached",
            "relation_id": f"rel_old_{index}",
        }
        for index in range(AUTO_MAX_LINKS_PER_BUCKET)
    ]
    incoming = {**_auto("new-active", 0.95), "relation_id": "rel_new"}

    merged = merge_auto_links(detached, [incoming])

    assert len(merged) == AUTO_MAX_LINKS_PER_BUCKET + 1
    assert sum(link["status"] == "active" for link in merged) == 1
...
@pytest.mark.asyncio
async def test_auto_relation_is_one_bidirectional_relation_with_shared_id(
    bucket_mgr, monkeypatch
):
    source_id = await bucket_mgr.create("source memory", title="Source")
    target_id = await bucket_mgr.create("target memory", title="Target")
    monkeypatch.setattr(rt, "bucket_mgr", bucket_mgr, raising=False)
    monkeypatch.setattr(rt, "logger", MagicMock(), raising=False)

    async def infer(_bucket_id: str, _content: str) -> list[dict]:
        return [_auto(target_id, 0.91, "same_event")]

    monkeypatch.setattr(relation_link, "infer_links_for", infer)

    assert await relation_link.link_new_bucket(source_id, "source memory") == 1
    source_link = (await bucket_mgr.get(source_id))["metadata"]["relation_links"][0]
    target_link = (await bucket_mgr.get(target_id))["metadata"]["relation_links"][0]

    assert source_link["relation_id"].startswith("rel_")
    assert source_link["relation_id"] == target_link["relation_id"]
    assert source_link["target_bucket_id"] == target_id
    assert target_link["target_bucket_id"] == source_id
    assert source_link["type"] == "same_event"
    assert target_link["type"] == "same_event"

    assert "status=detached" in await detach(source_id, 1)
    source_link = (await bucket_mgr.get(source_id))["metadata"]["relation_links"][0]
    target_link = (await bucket_mgr.get(target_id))["metadata"]["relation_links"][0]
    assert source_link["status"] == target_link["status"] == "detached"

    assert "status=active" in await restore(target_id, 1)
    source_link = (await bucket_mgr.get(source_id))["metadata"]["relation_links"][0]
    target_link = (await bucket_mgr.get(target_id))["metadata"]["relation_links"][0]
    assert source_link["status"] == target_link["status"] == "active"


@pytest.mark.asyncio
async def test_auto_relation_skips_both_sides_when_either_endpoint_is_full(
    bucket_mgr, monkeypatch
):
    source_id = await bucket_mgr.create("source memory", title="Source")
    target_id = await bucket_mgr.create("target memory", title="Target")
    ...
    existing = [
        {
            **_auto(f"existing-{index}", 0.8 + index / 100),
            "relation_id": f"rel_existing_{index}",
        }
        for index in range(AUTO_MAX_LINKS_PER_BUCKET)
    ]

    def seed(post):
        post["relation_links"] = existing
        return True, "seeded"

    await bucket_mgr.mutate_relation_links(target_id, seed)

    async def infer(_bucket_id: str, _content: str) -> list[dict]:
        return [_auto(target_id, 0.99, "same_event")]

    monkeypatch.setattr(relation_link, "infer_links_for", infer)

    assert await relation_link.link_new_bucket(source_id, "source memory") == 0
    assert "relation_links" not in (await bucket_mgr.get(source_id))["metadata"]
    assert (await bucket_mgr.get(target_id))["metadata"]["relation_links"] == existing


@pytest.mark.asyncio
async def test_detached_auto_history_allows_a_new_bidirectional_link(
    bucket_mgr, monkeypatch
):
    source_id = await bucket_mgr.create("source memory", title="Source")
    target_id = await bucket_mgr.create("target memory", title="Target")
    ...
    assert await relation_link.link_new_bucket(source_id, "source memory") == 1
    source_links = (await bucket_mgr.get(source_id))["metadata"]["relation_links"]
    target_links = (await bucket_mgr.get(target_id))["metadata"]["relation_links"]
    assert len(source_links) == 1
    assert len(target_links) == AUTO_MAX_LINKS_PER_BUCKET + 1
    assert source_links[0]["relation_id"] == target_links[-1]["relation_id"]


def test_auto_relation_time_distance_normalizes_iso_offsets_to_utc():
    utc = {"created": "2026-08-20T12:00:00Z"}
    new_york = {"created": "2026-08-20T08:00:00-04:00"}
    shanghai = {"created": "2026-08-20T20:00:00+08:00"}

    assert relation_link._hours_apart(utc, new_york) == pytest.approx(0.0)
    assert relation_link._hours_apart(utc, shanghai) == pytest.approx(0.0)
