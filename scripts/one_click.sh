...
export OMBRE_BUCKETS_DIR="${PWD}/buckets"
export OMBRE_STATE_DIR="${PWD}/state"
export OMBRE_CONFIG_PATH="${PWD}/config.yaml"
EOF
  if [[ "${FEATURE_SCOPE}" == "full" ]]; then
    cat >> start_local.sh <<'EOF'
...
EOF
  ...
  cat >> start_local.sh <<'EOF'
nohup "${PYTHON_CMD}" src/server.py > logs/ombre-brain.log 2>&1 &
echo $! > state/ombre-brain.pid
echo "Ombre-Brain started: http://127.0.0.1:8000/health"
EOF
  if [[ "${FEATURE_SCOPE}" == "full" ]]; then
    cat >> start_local.sh <<'EOF'
nohup "${PYTHON_CMD}" src/gateway.py > logs/ombre-gateway.log 2>&1 &
...
$brain = Start-Process -FilePath $PythonCmd -ArgumentList "src/server.py" -RedirectStandardOutput $brainOut -RedirectStandardError $brainErr -PassThru -WindowStyle Hidden
...
$gateway = Start-Process -FilePath $PythonCmd -ArgumentList "src/gateway.py" -RedirectStandardOutput $gatewayOut -RedirectStandardError $gatewayErr -PassThru -WindowStyle Hidden
