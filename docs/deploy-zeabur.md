...
- `src/server.py`：MCP / Dashboard / bucket API，容器内监听 `8000`
- `src/gateway.py`：OpenAI-compatible Gateway / 自动记忆注入，默认监听 `8010`
...
而完整版本需要同时运行 `src/server.py` 和 `src/gateway.py`，并且两边看到同一份 `/data` 和 `/state`。
...
```text
src/server.py 看到一套 buckets/state
src/gateway.py 看到另一套 buckets/state
...
```text
ombre-brain    -> python src/server.py   -> 8000
ombre-gateway  -> python src/gateway.py  -> 8010
```
...
```text
Start command: python src/gateway.py
Port: 8010
```
...
```text
service 1: ombre-brain    -> python src/server.py
service 2: ombre-gateway  -> python src/gateway.py
shared /data
shared /state
same config.yaml
```
...
```text
完整部署不是单服务 Zeabur 一键部署。它需要 `src/server.py` + `src/gateway.py` 两个服务，并共享 buckets/state/config。2C2G 可以勉强跑，但普通 Zeabur Git 部署更适合单服务，所以建议先选 MCP/Dashboard-only 或 Gateway-only 试跑。旧 bucket 可以迁移，记忆本体是 Markdown 文件；embedding/state 属于索引和运行态，最好在新环境重建。
```
