...
- **server.py** — MCP 服务装配入口。创建组件后调 `tools._runtime.init(...)` 注入依赖，再把 `tools/current/manifest.py` 的唯一 `TOOL_MANIFEST` 注册到一个 `FastMCP` 实例；对外只暴露单连接器 `/mcp`。历史的 `mcp_extra` 和 server 内 decorator wrappers 已移除。Streamable HTTP 使用无状态 JSON 响应；stdio 与 legacy SSE 保持可用。启动段调 `web.register_all(mcp)` 装配 HTTP 路由。工具业务逻辑归 `tools/`，HTTP 业务逻辑归 `web/`。
...
## 3. MCP 工具规格（单 manifest，共 31 个）

生产注册只有一个事实来源：`tools/current/manifest.py::TOOL_MANIFEST`。它把 P0luz 核心功能与 current/Yinglianchun 扩展组成一个 31 工具并集，通过同一个 `/mcp` 连接器暴露：

- 记忆读取：`breath` / `breath_search` / `breath_advanced` / `read_bucket` / `source_read` / `list_buckets_light`
...
Streamable HTTP、stdio、legacy SSE 与导入式 ASGI 启动应列出同一组工具。
...
| `/mcp/*` | — | 公开 | FastMCP 单连接器：canonical manifest 中的全部 31 个工具 |
...
| HTTP 模式下 Claude.ai 连不上 | `server.py` | `__main__` CORS 中间件；`_app = mcp.streamable_http_app()`（单连接器，31 个工具由 canonical manifest 注册）；URL 末尾必须 `/mcp` |
