...
from ombrebrain.them import ThemService, ThemStoreError
...
def _persist_quota(quota: int, service: ThemService) -> None:
    ...
    runtime_config = service.config
    if not isinstance(runtime_config, dict):
        raise TypeError("Them runtime config must be mutable")
    runtime_section = runtime_config.setdefault("them", {})
...
def register(mcp) -> None:
    ...
    service = sh.them_service
    if service is None:
        raise RuntimeError("Them web routes require an initialized service")
    ...
    def response_payload() -> dict[str, object]:
        ...
        return {
            ...
            "max_tokens_per_person": int(service.max_tokens_per_person),
        }
    ...
    async def set_them_setting(request: Request) -> Response:
        ...
        with commit_lock, tool_gate.lock:
            ...
            try:
                if enabled:
                    state = service.set_enabled(True, expected_revision=revision)
                    visible = tool_gate.sync(True)
                else:
                    visible = tool_gate.sync(False)
                    if visible:
                        raise RuntimeError("MCP tool removal failed")
                    state = service.set_enabled(False, expected_revision=revision)
    ...
    async def add_them_person(request: Request) -> Response:
        ...
        try:
            person = service.add_person(names)
    ...
    async def leave_them_note(request: Request) -> Response:
        ...
        try:
            person = service.leave_note(person_id, text)
        except ValueError as exc:
            return JSONResponse({"error": str(exc)}, status_code=400)
        except ThemStoreError:
            return JSONResponse({"error": "them 暂时不可用"}, status_code=503)
