...
def _normalize_current_config_request(
    ...
    if "mcp_auth_mode" in body:
        ...
        if mode not in {"oauth", "token"}:
            raise ValueError("mcp_auth_mode must be one of: oauth, token")
    ...
    return {
        ...
        "reflection": {
            ...
            "daily_chat_memory_mode": str(
                reflection.get("daily_chat_memory_mode") or "review"
            ),
...
def register(mcp) -> None:
    ...
    async def api_config_update(request: Request) -> Response:
        ...
        if "dehydration" in body:
            ...
            dehydrator = cast("Dehydrator | None", getattr(sh, "dehydrator", None))
            if dehydrator is not None:
                ...
                if dehydrator.api_available and dehydrator.api_format == "openai_compat":
                    try:
                        ...
                        _rollback_runtime()
                        return JSONResponse(
                            {"error": "dehydration reload failed"}, status_code=500
                        )
                else:
        ...
        if persist_requested:
            ...
            try:
                ...
                return JSONResponse(
                    {"error": "persist failed", "updated": updated},
                    status_code=500,
                )
