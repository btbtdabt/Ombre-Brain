...
def _resync_restored_module(
    *,
    service: Any,
    tool_gate: Any,
    label: str,
) -> str | None:
    """Refresh a restored module state and atomically align its MCP gate."""
    if service is None or tool_gate is None:
        return f"{label} 快照已恢复，但当前运行时没有 {label} 服务或工具门"

    with tool_gate.lock:
        # GitHub import replaces the SQLite file beneath the live store. Match
        # MigrateEngine's restore boundary before consulting persisted state.
        try:
        ...
        try:
            ...
            visible = tool_gate.sync(state.enabled)
            ...
            if state is not None and state.enabled:
                try:
                    ...
                    pass
            try:
                tool_gate.sync(False)
            except Exception:
                pass
            return f"恢复后的 {label} 开关未能生效，已按关闭处理"
    return None
    ...
    async def api_github_import(request: Request) -> Response:
        ...
        async with _import_lock:
            ...
            if result.pop("you_restored", False):
                you_service = sh.you_service
                you_tool_gate = sh.you_tool_gate
                sync_error = _resync_restored_module(
                    service=you_service,
            ...
            if result.pop("them_restored", False):
                them_service = sh.them_service
                them_tool_gate = sh.them_tool_gate
                sync_error = _resync_restored_module(
