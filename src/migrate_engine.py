        ...
        if "you/you.sqlite3" in names:
            ...
            try:
                if disk_backed:
                    ...
                    if not isinstance(you_source, bytes):
                        raise BackupArchiveError("You 内存快照不是字节数据")
        ...
        if "them/them.sqlite3" in names:
            ...
            try:
                if disk_backed:
                    ...
                    if not isinstance(them_source, bytes):
                        raise BackupArchiveError("them 内存快照不是字节数据")
    ...
    def _install_module_snapshot(
        ...
        *,
        ...
        try:
            ...
            校验(temp_path)
            os.replace(_win_long_path(Path(temp_path)), _win_long_path(Path(target)))
        finally:
        ...
        for _target_id, path in files.items():
            ...
            for link in links:
                ...
                if mapped and mapped != target_id:
                    ...
                    changed = True
                elif not mapped and link["status"] != "detached":
                    link["status"] = "detached"
                    changed = True
                    reason = (
                        "was present in the package but not imported"
                        if target_id in package_bucket_ids
                        else "was outside the imported package"
                    )
                    logger.warning(
                        "[migrate] detached Relation target %s in %s because it %s",
                        target_id,
                        _target_id,
                        reason,
                    )
