...
async def _preflight_access(bucket_id: str, expected_title: str) -> tuple[Any | None, str]:
...
async def attach(bucket_id: str, expected_title: str, source_content: str, source_ranges: Any = None) -> str:
    ...
    try:
        ...
        bucket, access_error = await _preflight_access(bucket_id, expected_title)
        if access_error:
            return access_error
    ...
    candidate = {"ref": candidate_ref, "ranges": ranges, "status": "active"}
    ...
    def mutation(post: Any) -> tuple[bool, str]:
        ...
        try:
            published_ref = rt.source_store.put(text)
        except (OSError, UnicodeError, ValueError) as exc:
            return False, f"source_attach 失败：{exc}"
        if published_ref != candidate_ref:
            return False, "source_attach 失败：Source 内容哈希不一致。"
    ...
    result = await rt.bucket_mgr.mutate_source_links(bucket_id, mutation)
    return result or f"未找到桶 {bucket_id}。"


async def _change_status(bucket_id: str, expected_title: str, source_slot: int, target: str) -> str:
    bucket_id = str(bucket_id or "").strip()
    expected_title = _normalized_title(expected_title)
...
async def detach(bucket_id: str, expected_title: str, source_slot: int) -> str:
    return await _change_status(bucket_id, expected_title, source_slot, "detached")


async def restore(bucket_id: str, expected_title: str, source_slot: int) -> str:
    return await _change_status(bucket_id, expected_title, source_slot, "active")
