...
def _source_evidence_title(content: str) -> str:
    """Give source-backed memories a stable title when analysis is unavailable."""

    first_line = next(
        (line.strip() for line in str(content or "").splitlines() if line.strip()),
        "",
    )
    return normalize_memory_title(first_line[:60])
...
async def grow_items(items: list, source_content: str = "", test_data: bool = False) -> str:
    ...
    async def _process_item(item: dict) -> dict:
        ...
        try:
            ...
            meta: dict[str, Any]
            if callable(default_analysis):
                meta = cast(dict[str, Any], default_analysis())
            else:
                meta = {
                    "domain": ["未分类"],
                    "valence": 0.5,
            ...
            if needs_analysis:
                try:
                    meta = cast(dict[str, Any], await rt.dehydrator.analyze(content_str))
                except Exception as e:
            ...
            final_title = explicit_title or inferred_title or (
                _source_evidence_title(content_str) if source_ref else ""
            )
        ...
        return {
            ...
            "title_missing": not bool(final_title),
        }
