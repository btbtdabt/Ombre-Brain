...
async def link_new_bucket(bucket_id: str, content: str) -> int:
    ...
    for link in inferred:
        ...
        forward = {**link, "relation_id": relation_id}
        reverse = {
            **forward,
        ...
        def _mutation(
            ...
            # Existing one-way links may be intentional V1/manual data.  Do not
            # duplicate or silently reinterpret them during automatic linking.
            if any(
                item["target_bucket_id"] == _target_id for item in left_links
            ) or any(
                item["target_bucket_id"] == _bucket_id for item in right_links
            ):
                return False, False, 0

            # A local score-based eviction cannot be atomic with the evicted
            # relation's mirror in a third bucket.  Append only when both ends
            # have capacity, preserving the bidirectional invariant.
            if not auto_link_has_capacity(left_links) or not auto_link_has_capacity(
                right_links
            ):
                return False, False, 0
            ...
            if _link not in merged_left or _reverse not in merged_right:
                return False, False, 0
