...
def _ordinary(bucket: Any, bucket_id: str = "") -> bool:
    ...
    return kind not in {
        ...
        "i_candidate",
        "identity",
    } and not is_letter_bucket({"metadata": meta})
...
async def attach(
    ...
    """Create one logical relation and persist mirrored local views."""
    ...
    if source and target and (
        not _ordinary(source, bucket_id) or not _ordinary(target, target_bucket_id)
    ):
        return "Relation only permits ordinary memory buckets."
    ...
    mirror = {
        ...
        "type": reverse_relation_type(relation_type),
    ...
    def mutation(source_post: Any, target_post: Any):
        ...
        if not _ordinary(target_post, target_bucket_id):
            return False, False, "Relation only permits ordinary memory buckets."
        try:
            source_links = normalize_relation_links(
                source_post.metadata.get("relation_links")
            )
            target_links = normalize_relation_links(
                target_post.metadata.get("relation_links")
            )
        except ValueError:
...
async def _change_legacy(
    ...
    return (
        await rt.bucket_mgr.mutate_relation_links(bucket_id, mutation)
    ) or f"未找到桶 {bucket_id}。"
...
async def _change(
    ...
    expected_title = _normalized_title(expected_title)
    if (
        isinstance(relation_slot, bool)
        or not isinstance(relation_slot, int)
        or relation_slot < 1
    ):
    ...
    if not _ordinary(source, bucket_id):
        return "Relation only permits ordinary memory buckets."
    try:
        source_links = normalize_relation_links(
    ...
    def mutation(source_post: Any, target_post: Any):
        ...
        if not _ordinary(target_post, target_bucket_id):
            return False, False, "Relation only permits ordinary memory buckets."
        try:
            left = normalize_relation_links(
                source_post.metadata.get("relation_links")
            )
            right = normalize_relation_links(
                target_post.metadata.get("relation_links")
            )
        except ValueError:
            return False, False, "任一端的 relation_links 格式无效，拒绝修改。"
...
async def detach(
    ...
    return await _change(bucket_id, relation_slot, "detached", expected_title)
...
async def restore(
    ...
    return await _change(bucket_id, relation_slot, "active", expected_title)
