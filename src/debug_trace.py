...
class DebugTraceLogger:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.max_value_chars = _int_value(
            os.environ.get("OMBRE_DEBUG_TRACE_MAX_VALUE_CHARS")
            or self.trace_cfg.get("max_value_chars"),
            12000,
            200,
            200000,
        )
        self.max_body_chars = _int_value(
            os.environ.get("OMBRE_DEBUG_TRACE_MAX_BODY_CHARS")
            or self.trace_cfg.get("max_body_chars"),
            20000,
            500,
            500000,
        )
    ...
    def _sanitize_value(self, value: Any, *, depth: int) -> Any:
        ...
        if isinstance(value, bytes):
            return self._clip(
                _redact_text(value.decode("utf-8", errors="replace")),
                self.max_body_chars,
            )
    ...
    def _prune_old_files(self) -> None:
        files = sorted(
            self.trace_dir.glob("trace-*.jsonl"),
...
class DebugTraceASGIMiddleware:
    ...
    async def __call__(self, scope: dict[str, Any], receive: Callable, send: Callable) -> None:
        ...
        request_payload = self.trace_logger.body_payload(
            bytes(request_body),
        ...
        try:
            ...
            response_payload = self.trace_logger.body_payload(
                bytes(response_body),
            ...
            self.trace_logger.write(
                ...
                payload={
                    ...
                    "jsonrpc": self._jsonrpc_summary(response_payload),
                },
                trace_id=trace_id,
            )
